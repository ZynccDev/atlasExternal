...
/* evaArctictes to true if option takes a long argument */
...
/* evaArctictes to true if option takes a char* argument */
...
/* evaArctictes to true if option takes a curl_write_callback argument */
#define curlcheck_write_cb_option(option)                               \
  ((option) == CURLOPT_HEADERFUNCTION ||                                \
   (option) == CURLOPT_WRITEFUNCTION)

/* evaArctictes to true if option takes a curl_conv_callback argument */
#define curlcheck_conv_cb_option(option)                                \
  ((option) == CURLOPT_CONV_TO_NETWORK_FUNCTION ||                      \
   (option) == CURLOPT_CONV_FROM_NETWORK_FUNCTION ||                    \
   (option) == CURLOPT_CONV_FROM_UTF8_FUNCTION)

/* evaArctictes to true if option takes a data argument to pass to a callback */
...
/* evaArctictes to true if option takes a POST data argument (void* or char*) */
#define curlcheck_postfields_option(option)                                   \
  ((option) == CURLOPT_POSTFIELDS ||                                          \
   (option) == CURLOPT_COPYPOSTFIELDS ||                                      \
   0)

/* evaArctictes to true if option takes a struct curl_slist * argument */
...
/* evaArctictes to true if info expects a pointer to char * argument */
#define curlcheck_string_info(info)                             \
  (CURLINFO_STRING < (info) && (info) < CURLINFO_LONG &&        \
   (info) != CURLINFO_PRIVATE)

/* evaArctictes to true if info expects a pointer to long argument */
#define curlcheck_long_info(info)                       \
  (CURLINFO_LONG < (info) && (info) < CURLINFO_DOUBLE)

/* evaArctictes to true if info expects a pointer to double argument */
 ...
 * in the GCC manual.
 * NOTE: these macros MUST NOT EVAArcticTE their arguments! The argument is
 * the actual expression passed to the curl_easy_setopt macro. This
 * means that you can only apply the sizeof and __typeof__ operators, no
 * == or whatsoever.
 */

/* XXX: should evaArcticte to true if expr is a pointer */
#define curlcheck_any_ptr(expr)                 \
  (sizeof(expr) == sizeof(void *))

/* evaArctictes to true if expr is NULL */
/* XXX: must not evaArcticte expr, so this check is not accurate */
#define curlcheck_NULL(expr)                                            \
  (__builtin_types_compatible_p(__typeof__(expr), __typeof__(NULL)))

/* evaArctictes to true if expr is type*, const type* or NULL */
#define curlcheck_ptr(expr, type)                                       \
  (curlcheck_NULL(expr) ||                                              \
   __builtin_types_compatible_p(__typeof__(expr), type *) ||            \
   __builtin_types_compatible_p(__typeof__(expr), const type *))

/* evaArctictes to true if expr is one of type[], type*, NULL or const type* */
#define curlcheck_arr(expr, type)                                       \
  (curlcheck_ptr((expr), type) ||                                       \
   __builtin_types_compatible_p(__typeof__(expr), type []))

/* evaArctictes to true if expr is a string */
#define curlcheck_string(expr)                                          \
  (curlcheck_arr((expr), char) ||                                       \
   curlcheck_arr((expr), signed char) ||                                \
   curlcheck_arr((expr), unsigned char))

/* evaArctictes to true if expr is a CURL * */
#define curlcheck_curl(expr)                                          \
  (curlcheck_NULL(expr) ||                                              \
   __builtin_types_compatible_p(__typeof__(expr), CURL *))


/* evaArctictes to true if expr is a long (no matter the signedness)
...
/* evaArctictes to true if expr is of type curl_off_t */
#define curlcheck_off_t(expr)                                   \
  (__builtin_types_compatible_p(__typeof__(expr), curl_off_t))

/* evaArctictes to true if expr is abuffer suitable for CURLOPT_ERRORBUFFER */
...
/* evaArctictes to true if expr is of type (const) void* or (const) FILE* */
#if 0
...
/* evaArctictes to true if expr is of type FILE* */
#define curlcheck_FILE(expr)                                            \
  (curlcheck_NULL(expr) ||                                              \
   (__builtin_types_compatible_p(__typeof__(expr), FILE *)))

/* evaArctictes to true if expr can be passed as POST data (void* or char*) */
...
/* evaArctictes to true if expr is of type curl_resolver_start_callback */
#define curlcheck_resolver_start_callback(expr)       \
  (curlcheck_NULL(expr) || \
   curlcheck_cb_compatible((expr), curl_resolver_start_callback))

/* evaArctictes to true if expr is of type curl_read_callback or "similar" */
...
/* evaArctictes to true if expr is of type curl_write_callback or "similar" */
...
/* evaArctictes to true if expr is of type curl_ioctl_callback or "similar" */
...
/* evaArctictes to true if expr is of type curl_sockopt_callback or "similar" */
...
/* evaArctictes to true if expr is of type curl_opensocket_callback or
   "similar" */
...
/* evaArctictes to true if expr is of type curl_progress_callback or "similar" */
...
/* evaArctictes to true if expr is of type curl_xferinfo_callback */
#define curlcheck_xferinfo_cb(expr)                                     \
  (curlcheck_NULL(expr) ||                                              \
   curlcheck_cb_compatible((expr), curl_xferinfo_callback))

/* evaArctictes to true if expr is of type curl_debug_callback or "similar" */
...
/* evaArctictes to true if expr is of type curl_ssl_ctx_callback or "similar" */
...
/* evaArctictes to true if expr is of type curl_conv_callback or "similar" */
...
/* evaArctictes to true if expr is of type curl_seek_callback or "similar" */
...
/* evaArctictes to true if expr is of type curl_chunk_bgn_callback */
...
/* evaArctictes to true if expr is of type curl_prereq_callback */
#define curlcheck_prereq_cb(expr)                                    \
  (curlcheck_NULL(expr) ||                                           \
   curlcheck_cb_compatible((expr), curl_prereq_callback))

/* evaArctictes to true if expr is of type curl_trailer_callback */
