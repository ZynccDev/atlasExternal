...
inline void RenderESP(ImDrawList* drawList)
{
    ...
    for (auto& player : Globals::Caches::CachedPlayerObjects)
    {
        ...
        if (Options::ESP::Box)
        {
            if (left < right && top < bottom)
            {
                ...
                case 2: // Corner
                {
                    ...
                    //drawList->AddRectFilled(ImVec2(left, top), ImVec2(right, bottom), boxFilledColor, 0.0f, 0);
        ...
        if (Options::ESP::TargetCircle)
        {
			if (Options::Aimbot::Aimbot)
            {
                ImColor targetCircleColor = IM_COL32(
                ...
                drawList->AddCircleFilled(ImVec2(head2D.x, head2D.y), circleRadius, targetCircleColor);
        ...
        if (Options::ESP::Name)
        {
            ...
            ImVec2 namePos(newHeadName.x - textSize.x / 4.0f, newHeadName.y);
            drawList->AddText(font, fontSize, namePos, color, nameStr.c_str());
        }

        /* i dont feel like fixing ts ngl
        if (Options::ESP::Tool) {
            std::string toolName = player.Tool.Name();
            std::string toolStr = "[" + toolName + "]";
            float fontSize = 12.f * scale;
            ImVec2 textSize = font->CalcTextSizeA(fontSize, FLT_MAX, 0.f, toolStr.c_str());

            ImVec2 toolPos(head2D.x - textSize.x / 4.0f, newLeftLeg.y);
            drawList->AddText(font, fontSize, toolPos, toolColor, toolStr.c_str());
        }*/

        if (Options::ESP::Distance)
        {
            std::ostringstream oss;
            oss << std::fixed << std::setprecision(2) << roundedDistance << " Studs";
            std::string distStr = "[" + oss.str() + "]";
            float fontSize = 12.f * scale;
        ...
        if (Options::ESP::Health)
        {
            ...
            case 0: // Normal
            {
                ...
                float barWidth = std::clamp(scale * 4.f, 3.f, 8.f);
                float boxHeight = bottom - top;
        ...
        if (Options::ESP::Crosshair)
        {
            ...
            float fovRadiusX = 10.0f;
            float fovRadiusY = 10.0f;

           drawList->AddLine(ImVec2(center.x - fovRadiusX, center.y), ImVec2(center.x + fovRadiusX, center.y), crosshairColor, 1.0f);
           drawList->AddLine(ImVec2(center.x, center.y - fovRadiusY), ImVec2(center.x, center.y + fovRadiusY), crosshairColor, 1.0f);
        }
    }
}
